...
const SUPABASE_KEY = 'sb_publishable_LCKuoYEaj6uJ4SOTUkHKwA_CYXZYOjf'; // ← usando la key publica
...
async function logAccessIP() {
  if (window.location.protocol === 'file:') {
    console.warn('Running from file:// - skipping IP logging to prevent CORS errors');
    return;
  }
  ...
  try {
    ...
    console.error("Error al registrar IP", err);
  }
}

// Fin de lógica de auditoría

/* ── BOTON DE SINCRONIZACION MANUAL (REALTIME) ── */
async function forceSync() {
  ...
  // Mostrar barra roja de progreso
  statusBar.classList.add('active');
  progressBar.style.width = '15%';
  statusMsg.innerText = "Sincronización Iniciada - Esperando al Bot...";

  try {
    // 1. Insertar el comando en Supabase
    const { data, error } = await sbClient
      .from('comandos_bot')
    ...
    // 2. Suscribirse a los cambios en ese registro para saber cuándo termina
      ...
      .on(
        ...
        (payload) => {
          ...
          if (status === 'processing') {
            ...
            statusMsg.innerText = "Bot procesando datos de HQ Rentals...";
          }

          if (status === 'completed') {
            progressBar.style.width = '100%';
            statusMsg.innerText = "PROCESO DE SINCRONIZACIÓN FINALIZADO";
            text.innerText = "¡Listo!";
            icon.classList.remove('spin');
            
            // Recargar datos en la UI y luego refrescar pgina
            setTimeout(() => {
                location.reload();
            }, 2000);
    ...
    icon.classList.remove('spin');
    showError("Error de DB: " + (err.message || JSON.stringify(err) || "No se pudo conectar al bot"));
  }
}
