...
def fetch_reservations_range(fecha_desde: str, fecha_hasta: str, brand_id: int) -> pd.DataFrame:
    ...
    Usa paginación completa para garantizar que se obtienen TODOS los registros del rango.
    """
    ...
    data = _safe_get_all_pages(url, params)
    total = len(data.get("data", []))
    print(f"fetch_reservations_range -> {total} registros obtenidos (todas las paginas)")
...
def fetch_returns_range(fecha_desde: str, fecha_hasta: str, brand_id: int) -> pd.DataFrame:
    ...
    IMPORTANTE: No se limitan los fields para conservar total_days, total_paid y
    cancellation_reason_id que son necesarios para las métricas del reporte.
    Usa paginación completa.
    """
    url = f"{HQ_API_BASE}/car-rental/reservations"
    params = {
        "filters": json.dumps([
    ...
    data = _safe_get_all_pages(url, params)
    total = len(data.get("data", []))
    print(f"fetch_returns_range -> {total} registros obtenidos (todas las paginas)")
    return pd.DataFrame(data.get("data", []))
...
def _safe_get_all_pages(url: str, params: dict = None) -> dict:
    """
    Igual que _safe_get pero maneja paginación automáticamente.
    Retorna SIEMPRE un dict {"data": [...]} sin importar el formato de la API.

    La API de HQ puede responder de dos formas:
      - Lista plana : [{...}, {...}, ...]           <- sin paginación, todo en uno
      - Dict paginado: {"data":[...], "meta":{...}} <- con paginación Laravel
    """
    all_records: list = []
    page = 1
    ...
    while True:
        ...
        try:
            ...
            log.error("Timeout al conectar con %s (página %d)", url, page)
            break
        except requests.exceptions.HTTPError as e:
            log.error("HTTP error %s — %s (página %d)", e.response.status_code, url, page)
            break
        except requests.exceptions.ConnectionError:
            log.error("Error de conexión con %s (página %d)", url, page)
            break
        except (ValueError, KeyError) as e:
            log.error("Error parseando JSON de %s (página %d): %s", url, page, e)
            break

        # ── La API de HQ devuelve lista plana directamente (sin wrapper {"data":...}) ──
        if isinstance(response, list):
            all_records.extend(response)
            log.info("API retornó lista plana — %d registros totales", len(all_records))
            break  # lista plana = todo en una sola respuesta, no hay más páginas

        # ── Tipo inesperado ──
        if not isinstance(response, dict):
            log.error("Respuesta inesperada de la API (tipo %s) en %s", type(response).__name__, url)
            break

        # ── Formato paginado estándar {"data": [...], "meta": {...}} ──
        ...
        log.info("Paginación: página %d — %d registros acumulados", page, len(all_records))
        ...
        if meta:
            ...
            if last_page and int(current_page) >= int(last_page):
                break
            elif last_page:
                page += 1
                continue
        ...
        # Heurística: menos de 100 registros = última página
...
def generate_range_report(
    ...
    if data_reserva.empty and data_reserva2.empty:
    ...
    # ── Normalizar columnas numéricas en data_reserva (pick_up en rango) ──
    ...
    # ── Normalizar columnas numéricas en data_reserva2 (return en rango) ──
    if not data_reserva2.empty:
        for col in ("total_days", "total_paid"):
            if col in data_reserva2.columns:
                data_reserva2[col] = pd.to_numeric(data_reserva2[col], errors="coerce").fillna(0)
            else:
                data_reserva2[col] = 0

    # ── Conteos de estado (basados en pick_up_date en el rango) ──
    ...
    # ── Métricas financieras: unir ambos datasets eliminando duplicados por id ──
    # data_reserva : iniciaron en el rango (pick_up_date en rango)
    # data_reserva2: retornaron en el rango (return_date en rango), pueden incluir
    #   reservas cuyo pick_up fue ANTES del rango (no están en data_reserva)
    if (
        not data_reserva2.empty
        and "id" in data_reserva.columns
        and "id" in data_reserva2.columns
    ):
        ids_en_reserva = set(data_reserva["id"].tolist())
        # Solo las completadas/rental cuyo pick_up fue antes del rango (externas)
        rentals_externos = data_reserva2[
            (data_reserva2["status"].isin(["rental", "completed"])) &
            (~data_reserva2["id"].isin(ids_en_reserva))
        ]
        # Unir: las que iniciaron en el rango (rental/completed) + las externas
        rentals_df = pd.concat([
            data_reserva[data_reserva["status"].isin(["rental", "completed"])],
            rentals_externos,
        ], ignore_index=True)
    else:
        # Fallback: solo las que iniciaron en el rango con status rental
        rentals_df = data_reserva[data_reserva["status"] == "rental"]

    total_rental_days = rentals_df["total_days"].sum()
    total_paid_sum    = rentals_df["total_paid"].sum()
    avg_rate          = total_paid_sum / total_rental_days if total_rental_days else 0.0

    # ── Returns del período (return_date en el rango) ──
    if not data_reserva2.empty:
        returns   = len(data_reserva2[data_reserva2["status"].isin(["completed", "rental"])])
