...
def fetch_reservations_range(fecha_desde: str, fecha_hasta: str, brand_id: int) -> pd.DataFrame:
    ...
    IMPLEMENTACIÓN CORREGIDA: Elimina duplicados y maneja paginación correctamente.
    """
    print(f"**************************-----> {fecha_desde}, {fecha_hasta}, {brand_id}")
    url = f"{HQ_API_BASE}/car-rental/reservations"
    
    all_data = []
    seen_ids = set()  # Para rastrear IDs únicos y evitar duplicados
    offset = 0
    page_size = 100
    max_pages = 4
    ...
    for page in range(max_pages):
        ...
        if not page_data:
            print(f"   No hay más datos en la página {page + 1}, deteniendo paginación")
            break
        
        # Filtrar duplicados ANTES de agregar a all_data
        new_unique_data = []
        ...
        if duplicates_in_page > 0:
            print(f"   ⚠️ Se eliminaron {duplicates_in_page} duplicados de la página {page + 1}")
        
        all_data.extend(new_unique_data)
        
        # Si tenemos menos de page_size únicos, ya no hay más páginas
        if len(page_data) < page_size:
            print(f"   Última página alcanzada con {len(page_data)} registros")
            break
            
        offset += page_size
        
        # Si ya tenemos suficientes registros únicos (300+), podemos parar
        if len(all_data) >= 300:
            print(f"   Obtenidos {len(all_data)} registros únicos, límite mínimo alcanzado")
            break
    
    print(f"   Total de registros únicos obtenidos: {len(all_data)}")
    print(f"   Total de duplicados eliminados: {len(seen_ids) - len(all_data)}")
...
def fetch_returns_range(fecha_desde: str, fecha_hasta: str, brand_id: int) -> pd.DataFrame:
    ...
    for page in range(max_pages):
        ...
        # Si tenemos menos de page_size únicos, ya no hay más páginas
        if len(page_data) < page_size:
            print(f"   Returns última página alcanzada con {len(page_data)} registros")
            break
        ...
        # Si ya tenemos suficientes registros únicos (300+), podemos parar
        if len(all_data) >= 300:
            print(f"   Returns obtenidos {len(all_data)} registros únicos, límite mínimo alcanzado")
            break
    
    print(f"   Returns total de registros únicos obtenidos: {len(all_data)}")
    print(f"   Returns total de duplicados eliminados: {len(seen_ids) - len(all_data)}")
...
# ─────────────────────────────────────────────
# REPORTE POR RANGO DE FECHAS (CORREGIDO)
# ─────────────────────────────────────────────
def generate_range_report(
    ...
    print(f"Generando reporte para {location} del {fecha_desde} al {fecha_hasta}")
    ...
    print(f"Estadísticas para {location}: onrent={onrent}, open={openr}, returns={returns}, brand_id={brand_id}")
...
# ─────────────────────────────────────────────
# ENVÍO COMPLETO A TELEGRAM - RANGO DE FECHAS (CORREGIDO)
# ─────────────────────────────────────────────
...
# ─────────────────────────────────────────────
def main() -> None:
    print("\n🤖 NOVA - Bot de Rent-A-Car HQ (CORREGIDO)")
    print("=" * 40)
